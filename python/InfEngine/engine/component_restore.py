"""
Shared Python component restoration logic for InfEngine.
...
from InfEngine.engine.project_context import resolve_script_path, resolve_guid_to_path
    ...
    if not error_msg:
        error_msg = (
            ...
            if not script_path
                ...
                f"Script '{script_path}' contains no InfComponent subclass "
                f"for '{pc.type_name}'"
            )
        )
...
def restore_single_component(scene, pc, asset_database=None):
    ...
    On success the new :class:`InfComponent` is attached to its
...
def restore_pending_py_components(
    ...
    *,
    ...
    clear_registries :
        If ``True``, clears :class:`InfComponent` active-instance and
