...
const GATEWAY_ORIGIN = "https://community-api.infernux-engine.com";
...
function gatewayRequest(path, options = {}) {
    ...
    return new Request(`${GATEWAY_ORIGIN}${path}`, { ...options, headers });
}

function sameOriginGatewayRequest(path, options = {}) {
    const headers = new Headers(options.headers || {});
    headers.set("Origin", SITE_ORIGIN);
    return new Request(`${SITE_ORIGIN}/community-api${path}`, { ...options, headers });
}
...
test("public Atom feed remains readable without a GitHub API token", async () => {
    const originalFetch = globalThis.fetch;
    let requestedUrl = "";
    try {
        globalThis.fetch = async (url) => {
            requestedUrl = String(url);
            return new Response('<?xml version="1.0"?><feed xmlns="http://www.w3.org/2005/Atom"><title>Recent discussions</title></feed>', {
                headers: { "Content-Type": "application/atom+xml" }
            });
        };
        const response = await worker.fetch(gatewayRequest("/api/discussions-feed"), {
            SITE_ORIGIN,
            SITE_ORIGINS,
        ...
        assert.equal(requestedUrl, "https://github.com/ChenlizheMe/Infernux/discussions.atom");
        assert.match(await response.text(), /Recent discussions/);
    } finally {
        globalThis.fetch = originalFetch;
    }
});

test("same-origin route preserves the gateway prefix for generated URLs", async () => {
    const request = sameOriginGatewayRequest("/health?probe=1");
    const response = await worker.fetch(request, { SITE_ORIGIN, SITE_ORIGINS }, workerContext());
    assert.equal(response.status, 200);
    assert.equal(await response.text(), "ok");
    assert.equal(internals.requestPath(request), "/health");
    assert.equal(internals.gatewayUrl(request, "/media/example.png"), `${SITE_ORIGIN}/community-api/media/example.png`);
});

test("OAuth uses the gateway host as its explicit callback", async () => {
    const response = await worker.fetch(gatewayRequest("/oauth/start"), {
    ...
    assert.equal(authorize.searchParams.get("redirect_uri"), `${GATEWAY_ORIGIN}/oauth/callback`);
    assert.equal(internals.oauthRedirectUri(gatewayRequest("/oauth/start")), `${GATEWAY_ORIGIN}/oauth/callback`);

    const sameOriginResponse = await worker.fetch(sameOriginGatewayRequest("/oauth/start"), {
        GITHUB_CLIENT_ID: "github-app-client-id"
    }, workerContext());
    const sameOriginAuthorize = new URL(sameOriginResponse.headers.get("Location"));
    assert.equal(sameOriginAuthorize.searchParams.get("redirect_uri"), `${SITE_ORIGIN}/community-api/oauth/callback`);
});
