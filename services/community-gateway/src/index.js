...
const MAX_ATOM_FEED_BYTES = 512 * 1024;
...
function requestPath(request) {
    ...
    if (pathname === SAME_ORIGIN_PREFIX) return "/";
    return pathname.startsWith(`${SAME_ORIGIN_PREFIX}/`) ? pathname.slice(SAME_ORIGIN_PREFIX.length) : pathname;
}

function gatewayUrl(request, pathname) {
    const source = new URL(request.url);
    const prefixed = source.pathname === SAME_ORIGIN_PREFIX || source.pathname.startsWith(`${SAME_ORIGIN_PREFIX}/`);
    source.pathname = `${prefixed ? SAME_ORIGIN_PREFIX : ""}${pathname}`;
    source.search = "";
    source.hash = "";
    return source.href;
}
...
async function discussionAtomFeed(env, origin) {
    const url = `https://github.com/${env.GITHUB_OWNER}/${env.GITHUB_REPO}/discussions.atom`;
    try {
        const response = await fetch(url, {
            headers: {
            ...
            cf: { cacheEverything: true, cacheTtl: ANONYMOUS_FEED_CACHE_SECONDS }
        });
        if (!response.ok) return errorResponse("github_unavailable", `GitHub feed returned ${response.status}.`, 502, origin);
        const feed = await response.text();
        if (utf8(feed).byteLength > MAX_ATOM_FEED_BYTES || !/<feed\b[^>]*xmlns=["']http:\/\/www\.w3\.org\/2005\/Atom["']/i.test(feed)) {
            return errorResponse("invalid_feed", "GitHub returned an invalid discussion feed.", 502, origin);
        }
        ...
        return errorResponse("github_unavailable", "The public discussion feed is unavailable.", 502, origin);
    }
}
...
async function uploadImage(request, env, origin, session) {
    ...
    const mediaUrl = gatewayUrl(request, `/media/${key}`);
...
function oauthRedirectUri(request) {
    return gatewayUrl(request, "/oauth/callback");
}
...
async function apiRequest(request, env, context) {
    ...
    if (path === "/api/session/refresh" && request.method === "POST") return refreshSession(request, env, origin);
    if (path === "/api/discussions-feed" && request.method === "GET") return discussionAtomFeed(env, origin);
    if (path === "/api/discussions" && request.method === "GET") return listDiscussions(request, env, origin, session, context);
    if (path === "/api/discussions" && request.method === "POST") return createDiscussion(request, env, origin, session);
    if (path === "/api/uploads" && request.method === "POST") return uploadImage(request, env, origin, session);
    const detail = path.match(/^\/api\/discussions\/([1-9]\d{0,9})$/);
...
export const internals = Object.freeze({ discussionCacheRequest, gatewayUrl, imageType, normalizeDiscussionInput, oauthRedirectUri, openSession, requestPath, sealSession });
...
export default {
    async fetch(request, env, context) {
        ...
        if (path === "/oauth/start" && request.method === "GET") return oauthStart(request, env);
        if (path === "/oauth/callback" && request.method === "GET") return oauthCallback(request, env);
        if (path.startsWith("/api/")) return apiRequest(request, env, context);
        if (path.startsWith("/media/") && request.method === "GET") return serveImage(request, env, path.slice(7));
