...
const apiPath = path.resolve(scriptDir, "..", "js", "community-api-v5.js");
...
const gatewayHealthUrl = new URL(`${config.gatewayPath}/health`, "https://infernux-engine.com");
const gatewayResponse = await fetch(gatewayHealthUrl, { signal: AbortSignal.timeout(15_000) });
const gatewayBody = await gatewayResponse.text();
validateCommunityGateway(gatewayResponse.status, gatewayBody);
console.log(`PASS same-origin community gateway at ${gatewayHealthUrl.origin}${gatewayHealthUrl.pathname}`);
