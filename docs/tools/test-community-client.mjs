...
const apiSource = await readFile(path.join(docsRoot, "js", "community-api-v5.js"), "utf8");
...
function createApiSandbox(initialStorage, fetchImpl) {
    const persistent = new Map(initialStorage);
    const tab = new Map();
    let assigned = "";
    const apiSandbox = {
        AbortSignal,
        ...
        localStorage: storageFor(persistent),
        sessionStorage: storageFor(tab),
        window: {
            location: {
                hash: "",
                search: "",
                href: "https://infernux-engine.com/community.html",
                hostname: "infernux-engine.com",
                origin: "https://infernux-engine.com",
    ...
    new vm.Script(apiSource, { filename: "community-api-v5.js" }).runInContext(apiSandbox);
    return { api: apiSandbox.InfernuxCommunityApi, persistent, assigned: () => assigned };
}
...
const legacySession = `v1.${"a".repeat(16)}.${"b".repeat(32)}`;
const sessionRequests = [];
const authenticatedApi = createApiSandbox([
    ["infernux-community-session-v1", legacySession]
], async (url) => {
    sessionRequests.push(url);
    return new Response(JSON.stringify({ authenticated: true, user: { login: "infernux" } }), {
        headers: { "content-type": "application/json" }
    });
});
assert.equal((await authenticatedApi.api.session()).authenticated, true);
assert.deepEqual(sessionRequests, ["https://community-api.infernux-engine.com/api/session"]);
assert.equal(authenticatedApi.persistent.get("infernux-community-gateway-v1"), "https://community-api.infernux-engine.com");

const signInRequests = [];
const anonymousApi = createApiSandbox([], async (url) => {
    signInRequests.push(url);
    return new Response(url.startsWith("https://infernux-engine.com/") ? "missing" : "ok", {
        status: url.startsWith("https://infernux-engine.com/") ? 404 : 200
    });
});
await anonymousApi.api.signIn();
assert.deepEqual(signInRequests, [
    "https://infernux-community.chenlizheme.workers.dev/health"
]);
assert.equal(anonymousApi.assigned(), "https://infernux-community.chenlizheme.workers.dev/oauth/start");
assert.equal(anonymousApi.persistent.get("infernux-community-gateway-v1"), "https://infernux-community.chenlizheme.workers.dev");

const feedRequests = [];
const feedApi = createApiSandbox([], async (url) => {
    feedRequests.push(url);
    if (url.startsWith("https://infernux-community.chenlizheme.workers.dev/")) {
...
assert.match(await feedApi.api.discussionFeed(), /<feed/);
assert.deepEqual(feedRequests, [
    "https://infernux-engine.com/community-api/api/discussions-feed",
    "https://community-api.infernux-engine.com/api/discussions-feed",
    "https://infernux-community.chenlizheme.workers.dev/api/discussions-feed"
]);
assert.equal(feedApi.persistent.get("infernux-community-gateway-v1"), "https://infernux-community.chenlizheme.workers.dev");
...
for (const contract of [
    ...
    'js/community-api-v5.js?v=1',
    'js/community.js?v=14',
...
for (const contract of [
    ...
    "parseCommunityAtomFeed",
    "gateway.discussionFeed(signal)",
...
for (const contract of [
    'FALLBACK_API_ORIGIN = "https://community-api.infernux-engine.com"',
    'WORKERS_API_ORIGIN = "https://infernux-community.chenlizheme.workers.dev"',
    'SAME_ORIGIN_PREFIX = "/community-api"',
    "PRODUCTION_HOSTS",
    "GATEWAY_KEY",
    "readPreferredGateway",
    "SESSION_PATTERN.test(existingSession)",
    "rememberGateway(origin)",
    "gatewayRequestUrl",
    "Community API paths must be root-relative",
    "gatewayFetch",
    "discussionFeed",
...
]) assert.ok(apiSource.includes(contract), `community-api-v5.js must retain '${contract}'`);
...
assert.doesNotMatch(apiSource, /_inx_gateway/, "the browser bundle must not retain the stale edge-cache query workaround");
assert.ok(communityHtml.includes("https://*.githubusercontent.com"), "community CSP must allow GitHub account avatars");

for (const contract of ['id="community-topic"', 'id="topic-body"', 'id="topic-replies"', 'data-mapping="number"', 'js/community-api-v5.js?v=1', 'js/community-topic.js?v=8']) {
    assert.ok(topicHtml.includes(contract), `community-topic.html must retain '${contract}'`);
}
for (const contract of ["normalizeCommunityTopicDetail", "DOMParser", "appendSafeCommunityNode", "COMMUNITY_TOPIC_TAGS", "requestCommunityTopic", "parseCommunityAtomTopic", "gateway.discussionFeed(signal)", "gateway.token?.()", 'mapping: "number"', 'url.pathname.startsWith("/community-api/media/")', 'hostname === "community-api.infernux-engine.com"', 'hostname === "infernux-community.chenlizheme.workers.dev"']) {
