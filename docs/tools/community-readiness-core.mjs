...
export function parseCommunityReadinessConfig(communityHtml, topicHtml, apiSource) {
    ...
    const gatewayOrigin = apiSource.match(/API_ORIGIN\s*=\s*["'](https:\/\/[^"']+)["']/)?.[1];
    if (!gatewayOrigin) throw new Error("community-api.js is missing the HTTPS community gateway origin");
    const config = {
