...
function atomEntryText(entry, localName) {
    return entry.getElementsByTagNameNS("*", localName)[0]?.textContent?.trim() || "";
}

function parseCommunityAtomFeed(value) {
    ...
    return [...documentNode.getElementsByTagNameNS("*", "entry")].map((entry) => {
        const alternate = [...entry.getElementsByTagNameNS("*", "link")]
            .find((link) => !link.getAttribute("rel") || link.getAttribute("rel") === "alternate");
        const url = alternate?.getAttribute("href") || "";
        const updatedAt = atomEntryText(entry, "updated");
        return {
            html_url: url,
            title: atomEntryText(entry, "title"),
            category: { name: "Discussion", slug: "discussion", is_answerable: false },
            user: { login: atomEntryText(entry, "name") || "unknown" },
            created_at: atomEntryText(entry, "published") || updatedAt,
            updated_at: updatedAt,
            comments: 0,
            reactions: 0,
            locked: false
...
async function requestCommunityPage(page, signal) {
    ...
    if (gateway) {
        ...
        if (page === 1 && typeof gateway.discussionFeed === "function") {
            try {
                const items = parseCommunityAtomFeed(await gateway.discussionFeed(signal));
                if (items.length) return { items, source: "public" };
            } catch (error) {
                console.warn("Forum feed unavailable; trying the final GitHub API fallback.", error);
            }
        }
        if (gateway.token?.()) {
            gatewayError.code ||= "authenticated_gateway_unavailable";
            throw gatewayError;
        }
