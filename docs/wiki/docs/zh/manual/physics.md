---
title: "物理"
description: "解释 Rigidbody 与 Collider 职责、动态和运动学移动、固定步力、碰撞/触发回调、层与物理查询。"
category: 手册
...
```text
[INX-DIAGRAM:timeline:固定步物理命令与回调流程]
fixed_update 命令
        │
        ▼
施加力 / 速度 / 运动学目标
        │
        ▼
物理模拟 → 接触与触发器求解 → enter / stay / exit 回调
        │
        └── 下一固定步（独立于渲染帧 delta_time）
```
